...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^blaze_") ; then
        ...
        export BUILD_NUMBER=$( (date +%s%N ; echo $BLAZE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        BLAZE_BUILD=
    fi
    export BLAZE_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${BLAZE_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Blaze-AOSP model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Blaze-AOSP GitHub
        T=$(gettop)
        cd $T > /dev/null
